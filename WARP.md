...
Dura is a background process that watches Git repositories and automatically commits uncommitted changes to special `dura/*` branches without impacting HEAD, the current branch, or the Git index. It's a Rust-based tool that provides "undo" across an entire repository by snapshotting changes every ~5 seconds.
...
- Creates commits on special `dura/<HEAD-hash>` branches
...
- Stored in `~/.config/dura/config.toml` (or `$DURA_CONFIG_HOME/config.toml`)
...
- `RuntimeLock` stored in `~/.cache/dura/runtime.db` (or `$DURA_CACHE_HOME/runtime.db`)
- Tracks running poller's PID to prevent multiple instances
- Used by `dura kill` to signal shutdown
...
- Compares against last known commit time (from dura branch or HEAD)
...
3. For each watched repo in config:
   ...
   - If changed: `snapshots::capture()` creates commit on `dura/<hash>` branch
...
- `dura` - spawns real dura subprocess with isolated `$DURA_HOME`
- `daemon` - facilitates working with `dura serve` via blocking `read_line()`

## Environment Variables

- `DURA_CONFIG_HOME` - Override config directory (default: `~/.config/dura`)
- `DURA_CACHE_HOME` - Override cache directory (default: `~/.cache/dura`)
- `DURA_VERSION_SUFFIX` - Appends to version string
- `DURA_PLAIN_TEXT` - Use plain ASCII symbols instead of Unicode
- `DURA_FANCY` - Force Unicode symbols
