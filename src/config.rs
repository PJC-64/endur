...
impl Config {
    ...
    pub fn default_path() -> Result<PathBuf> {
        Ok(Self::get_dura_config_home()?.join("config.toml"))
    }
    ...
    fn get_dura_config_home() -> Result<PathBuf> {
        ...
        dirs::config_dir()
            .map(|dir| dir.join("dura"))
            .ok_or_else(|| "Could not find your config directory. The default is ~/.config/dura but it can also be controlled by setting the DURA_CONFIG_HOME environment variable.".into())
    }
        ...
        if let Some(dir) = path.parent() {
            create_dir_all(dir)
                .map_err(|e| format!("Failed to create directory at `{}`: {}. \
                    ...
                    See https://github.com/tkellogg/dura for more information.", dir.display(), e, path.display()).into())
        } else {
            Ok(())
        }
    }
        ...
        for oid in revwalk.flatten() {
            if let Ok(commit) = repo.find_commit(oid) {
                if let Some(message) = commit.message() {
                    if message.ends_with("dura auto-backup") {
                        backup_count += 1;
